    ...
    switch (req) {
        ...
        case 'newTicket': {
            ...
            if (!channel) {
                ...
                ticketEmbed.setAuthor(`Seu tíquete foi criado com sucesso ${int.member.user.username}${reason ? ` (${reason})` : ''} ✅`);
                ticketEmbed.setDescription('*Para fechar o tíquete atual clique na reação abaixo, avisando que é impossível voltar atrás !*');
                ...
                return int.update({ content: `Seu tíquete está aberto <@${int.member.id}> <#${channel.id}> ✅`, components: [], ephemeral: true });
            } else {
                return int.update({ content: `Você já tem um tíquete aberto <#${channel.id}> ❌`, components: [], ephemeral: true });
            }
        }
        ...
        case 'saveTicket': {
            ...
            await channel.messages.fetch().then(async msg => {
                ...
                if (messages.length < 1) messages = 'Não há mensagens neste tíquete... strange';
